...
def receive_tick(result_list, stop_event):
    while not stop_event.is_set():
        data = redis_self.sub_msg('tickmodel')  # Redis에서 'modeltick' 키로부터 데이터를 수신합니다. 
...
def receive_news(result_list, stop_event):
    while not stop_event.is_set():
        data = redis_self.sub_msg('news')  # Redis에서 'modelnews' 키로부터 데이터를 수신합니다.
        result_list.append(data)  # 수신한 데이터를 'news_list'에 추가합니다.
        print(data)



def preprocess(last_tick_list, tick_list, last_kospi, last_news, kospi_list, news_list):
    
    x=150
    combined_list = last_tick_list + tick_list
    stock = pd.DataFrame(combined_list, columns=['date_time', 'open', 'high', 'low', 'close', 'volume', 'Accumulated sales volume', 'Accumulated number of contract'])
    print('체크1-8')
    # 이전의 kospi_list나 news_list 값으로 데이터프레임에 추가합니다.
    if last_kospi is not None:
        kospi_list.extend([last_kospi])
    if last_news is not None:
        news_list.extend([last_news])
...
#           stock.loc[:, 'trans_price_exp']=np.nan
    ...
    # 이전의 kospi_list나 news_list의 맨 마지막 값을 저장합니다.
    if kospi_list:
        last_kospi = kospi_list[-1]
    if news_list:
...
def load_data_v3_v4(code, ver):
    ...
    kospi_list = []
    news_list = []
    last_tick_list = []  # 이전 tick_list 값을 저장하기 위한 리스트
    ...
    x = 150
    # tick_list 값으로 데이터프레임을 구성합니다.
    chart_data = pd.DataFrame()
    
    training_data = pd.DataFrame()
    
    while len(tick_list) < 150:
        print('틱 데이터 {}/{}개'.format(len(tick_list), x))       
        
        current_time = datetime.datetime.now().time()  # 현재 시간을 datetime.time 객체로 가져오기
        if datetime.time(15, 20) <= current_time <= datetime.time(15, 21):
            
            if len(tick_list) == 0:
                time.sleep(60)
            print('중간에 전처리 시작')
            
            stock = preprocess(last_tick_list, tick_list, last_kospi, last_news, kospi_list, news_list)   
    ...
    print('체크1-5')
    y = 130
    # SELECT SQL 쿼리 실행 (최근 130개행을 가져오도록 ORDER BY와 LIMIT 절 사용)
    select_sql = "SELECT * FROM tick ORDER BY date_time DESC LIMIT {};".format(y)
    ...
    conn.close()
    print('체크1-6')
    # 각 튜플을 리스트로 변환하여 list_value_list에 저장
    for tpl in value_list:
        last_tick_list.append(tpl)
    #데이터 전처리
    stock = preprocess(last_tick_list, tick_list, last_kospi, last_news, kospi_list, news_list)   
    ...
    # edit_004170 데이터프레임의 각 행(row)에 대해 반복
    for row in tqdm(range(len(tick_list))):
        # INSERT SQL 쿼리를 생성하여 데이터를 데이터베이스에 삽입
        input_sql = "INSERT INTO tick VALUES {};".format(tuple(tick_list[row]))
        # SQL 쿼리 실행
        cur.execute(input_sql)
    # 변경 사항을 MySQL 서버에 확정 반영하기
    conn.commit()
    # 데이터베이스 연결 닫기
    conn.close()
