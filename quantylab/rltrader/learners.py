...
class ReinforcementLearner:
    ...
    def __init__(self, rl_method='rl', stock_code=None, 
                ...
                net='cnn', num_steps=5, lr=0.0006, 
                discount_factor=0.8, num_epoches=100,
...
#             self.reset()
            ...
            for i in tqdm(range(len(self.training_data)), leave=False):
                ...
                action, confidence, exploration = \
                    self.update_agent.decide_action(pred_value, pred_policy, epsilon)

                # 결정한 행동을 수행하고 보상 획득
                reward = self.update_agent.act(action, confidence)
                ...
                self.memory_pv.append(self.update_agent.portfolio_value)
                self.memory_num_stocks.append(self.update_agent.num_stocks)
                if exploration:
            ...
            logger.debug(f'[{self.stock_code}][Epoch {epoch_str}/{self.num_epoches}] '
                ...
                f'#Buy:{self.update_agent.num_buy} #Sell:{self.update_agent.num_sell} #Hold:{self.update_agent.num_hold} '
                f'#Stocks:{self.update_agent.num_stocks} PV:{self.update_agent.portfolio_value:,.0f} '
            ...
            max_portfolio_value = max(
                max_portfolio_value, self.update_agent.portfolio_value)
            if self.update_agent.portfolio_value > self.update_agent.initial_balance:
                epoch_win_cnt += 1
